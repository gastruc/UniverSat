...
from data.utils import (filter_labels_by_threshold,
                        subset_dict_by_filename, load_norm, apply_norm)
...
def replace_nans_with_mean(batch_of_images):
    if len(batch_of_images.shape) == 4:
        image_means = torch.nanmean(batch_of_images, dim=(2, 3), keepdim=True)
    else:
        image_means = torch.nanmean(batch_of_images, dim=(1, 2), keepdim=True)
...
class TreeSAT(Dataset):
    def __init__(
        ...
        Args:
            ...
            temporal_dropout (float): fraction of timestamps dropped per time series at train time
        """
        self.path = path
        ...
        data_path = path+"split/" + split + "_filenames.lst"
    ...
    def load_labels(self, classes):
        with open(self.path + "TreeSatBA_v9_60m_multi_labels.json") as file:
            jfile = json.load(file)
            subsetted_dict = subset_dict_by_filename(self.data_list, jfile)
            labels = filter_labels_by_threshold(subsetted_dict, 0.7)
    ...
    def __getitem__(self, i):
        ...
        output = {'label': torch.FloatTensor(self.labels[i]), 'name': name}
        ...
        with h5py.File(self.path + "sentinel-ts/" + '.'.join(name.split('.')[:-1]) + ".h5", 'r') as file:
            if 's1-asc' in self.modalities:
                ...
                output["s1-asc"] = torch.tensor(file["sen-1-asc-data"][:, :2, : ,:])
            ...
            if 's1-des' in self.modalities:
                ...
                output["s1-des"] = torch.tensor(file["sen-1-des-data"][:, :2, : ,:])
            ...
            if 's1' in self.modalities:
                ...
                s1_asc = torch.tensor(file["sen-1-asc-data"][:,:2,:,:])
                s1_des = torch.tensor(file["sen-1-des-data"][:,:2,:,:])
        ...
        if "s1-mono" in self.modalities:
            with rasterio.open(self.path + "sentinel/s1/60m/" + name) as f:
                numpy_array = f.read()
        ...
        if "s2-mono" in self.modalities:
            with rasterio.open(self.path + "sentinel/s2/60m/" + name) as f:
                numpy_array = f.read()
            numpy_array = numpy_array.astype(np.float32)
        ...
        for modality in self.modalities:
            output[modality] = replace_nans_with_mean(output[modality])
        ...
        out = self.transform(output, dataset_name='tsaits')
        for key in self.modalities:
            assert not torch.isnan(out[key]).any(), f"NaN in {key} for {name}"
