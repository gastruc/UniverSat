...
class PASTIS(Dataset):
    def __init__(
        ...
        Args:
            ...
            classif (bool): if True, build a multi-label tile classification
                target; otherwise return the dense segmentation map
            norm_path (str): directory holding per-modality normalisation statistics
            split (str): one of "train", "val", "test"
            temporal_dropout (int): max number of timestamps kept per time series
                at train/val time (0 disables dropout)
        """
    ...
    def __getitem__(self, i):
        ...
        for modality in self.modalities:
            if modality == "spot":
                ...
                if (self.split == "train" or self.split == "val") and N > self.temporal_dropout:
        ...
        return self.transform(output, dataset_name='pastis')
